#!/bin/bash
# Eval script for test16-dataset-mapping
...
echo "=========================================="
echo "EVAL: test16-dataset-mapping"
echo "=========================================="
...
if echo "$workflow_logs" | grep -q "Dataset mapping test"; then
    echo "PASS: Dataset mapping test log found"
else
    echo "FAIL: Dataset mapping test log NOT found"
    echo "$workflow_logs"
    exit 1
fi
...
if echo "$workflow_crd" | grep -q "claimName: testorg-testrepo-test15"; then
    echo "PASS: Mapped PVC claim name 'testorg-testrepo-test15' found"
else
    echo "FAIL: Mapped PVC claim name 'testorg-testrepo-test15' NOT found"
    echo "Checking for default claimName instead..."
    if echo "$workflow_crd" | grep -q "claimName: testorg-testrepo-test16"; then
        echo "FAIL: Found default claimName (mapping not applied)"
    fi
    exit 1
fi
...
echo ""
echo "=========================================="
echo "PASS: test16-dataset-mapping - All validations passed"
echo "=========================================="
exit 0