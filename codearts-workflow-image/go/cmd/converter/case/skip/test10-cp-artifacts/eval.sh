#!/bin/bash
# Eval script for test10-cp-artifacts (copy-script approach)
...
echo "=========================================="
echo "EVAL: test10-cp-artifacts (copy-script approach)"
echo "=========================================="
...
if [ -f "${artifact_dest}/test2.txt" ]; then
    content=$(cat "${artifact_dest}/test2.txt")
    if [ "$content" = "artifact content 2" ]; then
        echo "PASS: Artifact file test2.txt found with correct content"
    else
        echo "FAIL: Artifact file test2.txt has incorrect content: $content"
        exit 1
    fi
else
    echo "FAIL: Artifact file test2.txt NOT found at ${artifact_dest}/test2.txt"
    exit 1
fi
