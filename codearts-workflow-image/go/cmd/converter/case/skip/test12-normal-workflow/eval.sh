#!/bin/bash
# Eval script for test12-normal-workflow (copy-script approach)
...
echo "=========================================="
echo "EVAL: test12-normal-workflow (copy-script approach)"
echo "=========================================="

echo ""
echo "[1/5] Fetching workflow CRD..."
...
if echo "$workflow_crd" | grep -q "entrypoint: main"; then
    echo "PASS: entrypoint is main"
else
    echo "FAIL: entrypoint should be main"
    exit 1
fi

if echo "$workflow_crd" | grep -q "name: main-script"; then
    echo "PASS: main-script template found"
else
    echo "FAIL: main-script template NOT found"
    exit 1
fi

if echo "$workflow_crd" | grep -q "name: copy-script"; then
    echo "PASS: copy-script template found"
else
    echo "FAIL: copy-script template NOT found"
    exit 1
fi

if echo "$workflow_crd" | grep -q "name: output"; then
    echo "PASS: volumeClaimTemplates name 'output' found"
else
    echo "FAIL: volumeClaimTemplates name 'output' NOT found"
    exit 1
fi
...
echo ""
echo "[3/5] Verifying workflow completion..."
...
if [[ "${job_status}" == "Completed" ]]; then
    ...
    echo "PASS: Workflow status is Running (copy-script approach - workflow controller update may be delayed)"
else
    echo "FAIL: Workflow status is ${job_status}"
    exit 1
fi
...
if echo "$workflow_logs" | grep -q "copying artifacts...."; then
    echo "PASS: 'copying artifacts....' found in logs"
else
    echo "FAIL: 'copying artifacts....' NOT found in logs"
    exit 1
fi
